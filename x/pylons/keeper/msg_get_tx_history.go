...
func TxHistoryRequestHandler(w http.ResponseWriter, r *http.Request, ctx client.Context) {
	...
	if err != nil {
		...
		info, _ := json.Marshal(types.NewStandardError(types.GetErrorHistoryCode, types.GetErrorHistoryMsg))
		_, _ = w.Write(info)
		return
	}
	w.Header().Add(types.HTTPContentTypeKey, types.HTTPContentTypeVal)
...
func ValidateRequest(address string, limit, offset int64) *types.StandardError {
	if IsInvalidAddress(address) {
		return types.NewStandardError(codes.InvalidArgument.String(), types.AddressNotFound)
	}
	_, err := sdkTypes.AccAddressFromBech32(strings.Trim(address, "'"))
	if err != nil {
		return types.NewStandardError(codes.InvalidArgument.String(), types.AddressInvalid)
	}

	if IsInvalidNum(offset) {
		return types.NewStandardError(codes.InvalidArgument.String(), types.OffsetInvalid)
	}

	if IsInvalidNum(limit) {
		return types.NewStandardError(codes.InvalidArgument.String(), types.LimitInvalid)
	}
	return nil
}
